  ...
  return (
    <div className="max-w-[90rem] m-4 text min-[1445px]:m-auto relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-x-10 gap-y-12 sm:gap-y-4 sm:mb-4">
        <Calendario 
          eventsList={data.eventosList} 
          addNewEvent={data.addNewEvent} 
          dayClicked={dayClicked}
        />
        <div className="col-span-2 bg-blue-1 rounded-3xl ring-1 ring-[#11567D]">
          <h1 id="selectedDate" className="text-center text-[1.5rem] sm:text-[3rem] mt-4 mb-4 clr-blue-3 titulo">
            {selectedDate}
          </h1>
          <div className="overflow-y-auto scrollbar-hide h-[32rem]">
           
              <ul className="flex flex-col px-4 sm:px-12 gap-4 mb-4 pb-4" style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', overflow: 'hidden' }}>
                {data.eventosList.map((todo) => (
                  <Eventos 
                   // Asumiendo que cada `todo` tiene un identificador único `id`
                    cat={todo.categoria} 
                    titulo={todo.titulo} 
                    texto={todo.texto} 
                    image={todo.image} 
